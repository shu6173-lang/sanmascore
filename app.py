...
with tab1:
    ...
    auto_expected_key = f"auto_expected_{date_str}_{game_idx}"
    ...
    if auto_expected_key not in st.session_state:
        st.session_state[auto_expected_key] = None
    ...
    def auto_calc_pt(changed_key):
        ...
        # 自動欄の変更通知が来た場合：
        # 「こちらが自動で入れた予定値」と同じなら自動更新による再実行なので追従を維持。
        # 値が違えば、ユーザーが±/直接入力で触ったと判断して自動追従を解除。
        if auto_target == changed_key:
            expected = st.session_state.get(auto_expected_key)
        ...
        # 自動追従中：元の2人を何回・素早く押しても、その時点の最新値から再計算。
        if auto_target in pt_keys:
            ...
            st.session_state[auto_expected_key] = new_value
            st.session_state[auto_target] = new_value
            return

        # 自動欄を本人が触った後は、3人とも完全手入力。
    ...
    if not all_pt_entered:
        ...
        st.success("✨ ゲームPt合計が 0 になっています！", icon="✅")

    if st.button("➕ この半荘の結果を記録する", type="primary", use_container_width=True):
        if not all_pt_entered:
            st.error("エラー：ゲームPtを2人分入力してください。残り1人は自動計算されます。")
        elif total_pt != 0:
            st.error("エラー：3人のゲームPt合計が0になるように調整してください。")
        else:
            record = {
                "日付": date_str,
        ...
        for idx, r in enumerate(current_history):
            with delete_cols[idx % len(delete_cols)]:
                if st.button(
                    ...
                    # 削除前に、その日の最終チップを退避
                    keep_p1_chip = sum(int(rec["p1_chip"]) for rec in current_history)
                    keep_p2_chip = sum(int(rec["p2_chip"]) for rec in current_history)
                    keep_p3_chip = sum(int(rec["p3_chip"]) for rec in current_history)
                    ...
                    # 半荘を削除しても、その日のチップ結果は残す
                    if current_history:
                        current_history[0]["p1_chip"] = keep_p1_chip
                        current_history[0]["p2_chip"] = keep_p2_chip
                        current_history[0]["p3_chip"] = keep_p3_chip
